(function(){
  ...
  const I18N_FALLBACKS={
    ...
    desktop_pet_permission_allow:'Allow & continue',
    desktop_pet_permission_direct_send_body:'Desktop Companion can send this reply without another WebUI click after you allow it.',
    desktop_pet_permission_direct_send_safe:'Send as draft',
    desktop_pet_permission_direct_send_title:'Allow direct send?',
    desktop_pet_permission_inline_action_body:'Desktop Companion can answer approval and clarify cards after you allow it.',
    desktop_pet_permission_inline_action_safe:'Open in WebUI',
    desktop_pet_permission_inline_action_title:'Allow action responses?',
    ...
    desktop_pet_time_days_ago:'{0}d ago',
    desktop_pet_time_hours_ago:'{0}h ago',
    desktop_pet_time_just_now:'just now',
    desktop_pet_time_minutes_ago:'{0}m ago',
    desktop_pet_title:'Hermes Pet',
    desktop_pet_welcome_action:'Got it',
  ...
  function _attentionItems(){
    ...
    _cleanupResolvedActionResponses(items);
    return items;
  }
  ...
  function _actionBaselineForSid(sid){
    const row=sessions.find(item=>item&&item.session_id===sid)||{};
    return {
      message_count:Number(row.message_count||0),
      last_message_at:Number(row.last_message_at||row.updated_at||0)
    };
  }
  function _cleanupResolvedActionResponses(items){
    const active=new Set((Array.isArray(items)?items:[]).map(_actionPendingKey).filter(Boolean));
    for(const key of Object.keys(pendingActionResponses)){
      if(active.has(key)) continue;
      delete pendingActionResponses[key];
      delete clarifyDrafts[key];
      if(clarifyOtherKey===key) clarifyOtherKey='';
    }
  ...
  function _expandHtml(item){
    ...
    if(item.actionType==='clarify'){
      if(pending) return `<div class="pet-card-expand"><div class="expand-question">${_esc(_petT('desktop_pet_sending'))}</div></div>`;
      ...
      const otherSelected=!choices.length||clarifyOtherKey===pendingKey||!!clarifyDrafts[pendingKey];
